...
namespace DragonBall
{
    public class JobDriver_PlaceDragonBall : JobDriver
    {
        ...
        protected override IEnumerable<Toil> MakeNewToils()
        {
            ...
            IntVec3 targetPos = AltarBuilding.GetDragonBallPosition(DragonBall.def);

            yield return new Toil
            {
                initAction = () =>
                {
                    Thing blockingThing = targetPos.GetThingList(pawn.Map)
                        .FirstOrDefault(t => t.def.category == ThingCategory.Item);

                    if (blockingThing != null)
                    {
                        GenPlace.TryPlaceThing(blockingThing, targetPos, pawn.Map, ThingPlaceMode.Near);
                    }
                },
            ...
            yield return Toils_Goto.GotoCell(targetPos, PathEndMode.ClosestTouch)
            ...
            place.initAction = () =>
            {
                ...
                {
                    pawn.inventory.innerContainer.TryDrop(DragonBall, targetPos,
                        pawn.Map, ThingPlaceMode.Direct, out Thing droppedThing);
                }
            };
